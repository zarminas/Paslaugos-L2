    ...
    public class Subscriber : ISubscriber
    {
        ...
        public void Subscribe(Func<string, string, bool> callback)
        {
            ...
            consumer.Received += (sender, e) =>
            {
                ...
                bool success = callback.Invoke(message, e.RoutingKey);
                if (success)
                {
        ...
        public void SubscribeAsync(Func<string, string, Task<bool>> callback)
        {
            ...
            consumer.Received += async (sender, e) => 
            {
                var body = e.Body.ToArray();
                var message = Encoding.UTF8.GetString(body);
                bool success = await callback.Invoke(message, e.RoutingKey);
                if (success)
                {
                    _model.BasicAck(e.DeliveryTag, true);
                }
